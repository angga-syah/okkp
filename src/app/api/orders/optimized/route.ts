...
import { authOptions } from '@/lib/auth-config'; // ✅ Import shared authOptions
...
const validateAndSanitizeInput = (input: string, type: 'search' | 'general' = 'general'): string => {
...
// Validate date input
const validateDateInput = (dateStr: string | undefined): string | undefined => {
...
const validateNumericInput = (input: string | undefined, min: number, max: number): number => {
...
export async function GET(request: NextRequest) {
  ...
  try {
    // ✅ FIXED: Use authOptions with getServerSession
    ...
    // Validate and sanitize all inputs - convert null to undefined
    ...
    const rawSortField = searchParams.get('sortField') || undefined;
    ...
    const day = validateNumericInput(rawDay, 1, 31).toString();
    const month = validateNumericInput(rawMonth, 1, 12).toString();
    const year = validateNumericInput(rawYear, new Date().getFullYear() - 10, new Date().getFullYear() + 1).toString();
    ...
    const query: OrdersQuery = {
      ...
      dateFrom,
      dateTo,
      day: rawDay ? day : undefined,
      month: rawMonth ? month : undefined,
      year: rawYear ? year : undefined,
    };
    ...
    if (query.month && query.year && rawMonth && rawYear) {
    ...
    if (query.day && query.month && query.year && rawDay && rawMonth && rawYear) {
    ...
    const { data: orders, count, error } = await Promise.race([queryPromise, timeoutPromise]) as any;
