// app/api/upload-result/route.ts - FIXED DATABASE UPDATE
...
// 🔧 FIXED: Enhanced file cleanup function - cleans ALL result files
async function cleanupOldResultFiles(orderId: string): Promise<void> {
  try {
    console.log('🧹 Starting aggressive cleanup for order:', orderId);
    
    // First, get current database file path to help with debugging
    ...
    if (currentOrder?.result_file_path) {
      console.log('🧹 Current DB file path:', currentOrder.result_file_path);
    }
    
    // List all files in the order's result directory
    const { data: existingFiles, error: listError } = await supabaseAdmin.storage
      .from('documents')
      .list(`results/${orderId}`, {
        limit: 100,
        sortBy: { column: 'created_at', order: 'desc' }
      });

    if (listError) {
      console.warn('⚠️ Could not list existing files:', listError.message);
      return;
    }

    if (!existingFiles || existingFiles.length === 0) {
      console.log('📁 No existing files found in directory');
      return;
    }

    console.log('📁 Found files in storage:', existingFiles.map(f => ({
      name: f.name,
      size: f.metadata?.size,
      lastModified: f.updated_at || f.created_at
    })));

    // Delete ALL result files (be more aggressive)
    const filesToDelete = existingFiles
      .filter(file => {
        if (file.name === '.keep') {
          console.log('📁 Preserving .keep file');
          return false;
        }
        
        // Delete any file that looks like a result file
        if (file.name.startsWith('result_') || 
          ...
          console.log('📁 Marking for deletion (result file):', file.name);
          return true;
        }
        
        return false;
      })
      .map(file => `results/${orderId}/${file.name}`);

    if (filesToDelete.length > 0) {
      console.log('🗑️ Attempting to delete all old files:', filesToDelete);
      ...
      if (bulkDeleteError) {
        console.warn('⚠️ Bulk delete failed:', bulkDeleteError.message);
        console.log('🗑️ Trying individual delete...');
        
        // Try deleting files one by one for better error handling
        let deletedCount = 0;
        let failedCount = 0;
        ...
        for (const filePath of filesToDelete) {
          try {
            ...
            console.warn(`⚠️ Exception deleting ${filePath}:`, error);
            failedCount++;
          }
        }
        
        console.log(`✅ Individual delete summary: ${deletedCount} deleted, ${failedCount} failed`);
      } else {
        console.log(`✅ Bulk delete successful: ${filesToDelete.length} files deleted`);
      }
    } else {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 🔧 FIXED: Use encryption.ts method but make it unique per upload
    const timestamp = Date.now();
    const downloadPassword = generateDownloadPassword(sanitizedOrderId + '_' + timestamp);
    console.log('🔐 Generated new unique password for order:', sanitizedOrderId);

    // 🔧 FIXED: Clean up ALL old files BEFORE generating new filename
    await cleanupOldResultFiles(sanitizedOrderId);
    
    // 🔧 FIXED: Generate highly unique filename with multiple entropy sources + nano timestamp
    
    const nanoTime = process.hrtime.bigint().toString(); // High precision timestamp
    ...
    // 🔧 FIXED: Ultra-unique filename with nano-precision and random suffix
    const uniqueSuffix = Math.random().toString(36).substring(2, 8);
    const filename = `result_${sanitizedOrderId}_${timestamp}_${nanoTime.slice(-8)}_${randomString}_${userHash}_${uniqueSuffix}${finalExtension}`;
    ...
    if (encryptionEnabled) {
      try {
        ...
        const encryptionMetadataWithType = {
          ...
          uploadVersion: 2 // Simple version instead of large timestamp
        };
    ...
    // 🔧 FIXED: Upload with better retry logic and cache busting
    ...
    // 🔧 FIXED: Database update with only existing columns
    ...
    if (encryptionEnabled && encryptionMetadata) {
      ...
      // 🔧 FIXED: Use simple version number instead of timestamp to avoid integer overflow
      updateData.result_encryption_version = 2; // Simple version number
    }
    
    console.log('💾 Updating database with new file info...');
    
    // 🔧 FIXED: Use a more robust update with verification
    ...
    if (shouldSendEmail && orderData.email) {
      if (!SecurityValidator.validateEmail(orderData.email)) {
        ...
        try {
          ...
          console.log('📧 Email sent successfully with new password');
        } catch (emailError) {
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        metadata: {
          ...
          uploadTime: new Date().toISOString() // Use ISO string instead of timestamp
        },
    ...
    console.log('🎉 Upload process completed successfully with new unique password');
    ...
    return NextResponse.json({
      ...
      message: 'File hasil berhasil diunggah dengan password baru',
      // 🔧 FIXED: Return comprehensive debug info
      debug: {
        oldFileDeleted: !!orderData.result_file_path,
        oldFilePath: orderData.result_file_path,
        newFilePath: filePath,
        passwordChanged: true,
        newPasswordPreview: downloadPassword.substring(0, 3) + '***',
