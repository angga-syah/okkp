...
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 5MB to prevent document bombs
...
async function validateOrderUpload(orderId: string, clientIp: string): Promise<{ isValid: boolean; message?: string; order?: any }> {
  try {
    ...
    const rateLimitResult = await uploadRateLimiter.checkAndLimit(clientIp);
    if (!rateLimitResult.success) {
      await uploadRateLimiter.recordAttempt(clientIp, false, { 
        reason: 'rate_limited',
        orderId: orderValidation.sanitized 
      });
      
      const waitTime = rateLimitResult.nextAttemptDelay || rateLimitResult.lockoutDuration || 0;
      return { 
        isValid: false, 
        message: `Too many upload attempts. Please wait ${Math.ceil(waitTime / 60)} minutes before trying again.` 
      };
    }
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Parse the form data with size limit
    ...
    // Validate order and rate limiting
    const orderValidation = await validateOrderUpload(orderId, clientIp);
    if (!orderValidation.isValid) {
      return NextResponse.json({ 
        success: false, 
    ...
    // Save the file to disk with secure naming
    ...
    // Get language from order record
    ...
    // Update the order status and document path
    ...
    if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
      try {
        ...
        const emailRateCheck = await emailRateLimiter.checkAndLimit(clientIp);
