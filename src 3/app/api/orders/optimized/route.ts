// src/app/api/orders/optimized/route.ts - COMPLETE FIX
...
const validateAndSanitizeInput = (input: string | null, type: 'search' | 'general' = 'general'): string => {
  ...
  // 🔧 FIX: Trim and limit length based on type
  let sanitized = input.trim().slice(0, type === 'search' ? 200 : 100);
  
  if (!sanitized) return '';
  
  // 🔧 FIX: Enhanced SQL injection prevention
  const sqlPatterns = [
  ...
  // 🔧 FIX: Remove dangerous characters but keep search-friendly ones
  if (type === 'search') {
    // For search, allow more characters but escape dangerous ones
    sanitized = sanitized.replace(/[<>\"']/g, '');
  } else {
    // For general input, be more restrictive
    sanitized = sanitized.replace(/[<>\"'&]/g, '');
  }
...
const validateArrayInput = (
  ...
  if (typeof input === 'string') {
    ...
    // 🔧 FIX: Split by comma and handle empty strings
  ...
  // 🔧 FIX: Filter against allowed values if provided
...
const validateDateInput = (dateStr: string | null): string | undefined => {
  ...
  // 🔧 FIX: Support multiple date formats
  const dateFormats = [
  ...
  // 🔧 FIX: Convert to standard format and validate
  ...
  // 🔧 FIX: Check reasonable date range (expanded)
  const now = new Date();
...
// 🔧 FIXED: Enhanced numeric validation
...
// 🔧 FIXED: Define allowed statuses based on database schema
const getAllowedStatuses = (): string[] => {
  return [
    ...
    'cancelled',
    'payment_expired',
    // 🔧 FIX: Add variations that might exist
    'pending',
    'verified',
    'process',
    'revision',
...
// 🔧 FIXED: Enhanced search term escaping for PostgreSQL
const escapeSearchTerm = (term: string): string => {
  if (!term) return '';
  
  // 🔧 FIX: Escape PostgreSQL special characters
  return term
...
export async function GET(request: NextRequest): Promise<NextResponse<OrdersResponse | { error: string; message: string; code: string; timestamp: string }>> {
  ...
  try {
    ...
    // 🔧 FIXED: Enhanced parameter parsing and validation
    ...
    // 🔧 FIXED: Enhanced sort field validation
    ...
    // 🔧 FIXED: Sort direction validation
    const sortDirection: 'asc' | 'desc' = rawSortDirection === 'asc' ? 'asc' : 'desc';

    // 🔧 FIXED: Enhanced status array validation
    const allowedStatuses = getAllowedStatuses();
    const status = validateArrayInput(rawStatus, allowedStatuses);

    // 🔧 FIXED: Service array validation (no restrictions as services can vary)
    const service = validateArrayInput(rawService);

    // 🔧 FIXED: Enhanced search validation
    ...
    // 🔧 FIXED: Enhanced day/month/year validation
    ...
    // 🔧 FIXED: Enhanced Supabase query building
    ...
    // 🔧 FIXED: Enhanced status filtering with proper OR conditions
    if (status && Array.isArray(status) && status.length > 0) {
      ...
      if (validStatuses.length === 1) {
        ...
        // 🔧 FIX: Use proper OR syntax for multiple statuses
    ...
    // 🔧 FIXED: Enhanced service filtering
    if (service && Array.isArray(service) && service.length > 0) {
      ...
      if (validServices.length === 1) {
        ...
        // 🔧 FIX: Use proper OR syntax for multiple services
...
// 🔧 FIXED: Individual date component filters (more flexible)
if (dateFrom) {
...
// 🔧 FIX: Individual component filters - work independently
if (year && !month && !day) {
  ...
  // Day only filter across all months/years (probably not very useful, but supported)
  // This will find all records where the day of month matches
  supabaseQuery = supabaseQuery.eq('EXTRACT(day FROM created_at)', day);
  console.log('🔍 Applied day-only filter (all months/years):', day);
}

    // 🔧 FIXED: Enhanced search implementation with proper escaping
if (search && search.trim()) {
  ...
  try {
    ...
    // 🔧 FIX: Simple but safe escaping for PostgreSQL ILIKE
    ...
    // 🔧 FIX: Use individual ILIKE queries instead of OR for better stability
    ...
    if (isEmailSearch) {
      ...
      // 🔧 FIX: Safe general text search - use simpler OR syntax
      try {
    ...
    // 🔧 FIXED: Enhanced sorting with proper index utilization
    ...
    try {
      ...
      if (error) {
        ...
        return NextResponse.json({
          ...
          message: 'Query execution failed. Please try again or contact support.',
          code: 'DB_QUERY_FAILED',
          timestamp: new Date().toISOString()
        }, { status: 500 });
      }

      // 🔧 FIXED: Enhanced data validation and sanitization
      ...
      // 🔧 FIXED: Enhanced response structure
