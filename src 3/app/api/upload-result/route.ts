// app/api/upload-result/route.ts - FIXED VERSION
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 🔧 FIX: DELETE OLD FILE FIRST BEFORE UPLOADING NEW ONE
    const oldFilePath = orderData.result_file_path;
    if (oldFilePath) {
      console.log('🗑️ Deleting old result file:', oldFilePath);
      try {
    ...
    // 🔧 FIX: Generate unique filename with timestamp and random string to avoid collisions
    ...
    // 🔧 FIX: More unique filename pattern
    const filename = `result_${sanitizedOrderId}_${timestamp}_${randomString}${finalExtension}`;
    ...
    // 🔧 FIX: Upload with upsert: true to replace existing files
    ...
    while (retries < maxRetries) {
      const uploadResult = await supabaseAdmin.storage
        ...
        .upload(filePath, fileToUpload, {
          ...
          cacheControl: '3600',
          upsert: true // 🔧 FIX: Always upsert to replace existing files
        });
    ...
    // 🔧 FIX: Update database with new file info and clear cache-related fields
    ...
    if (encryptionEnabled && encryptionMetadata) {
      ...
      updateData.result_encryption_version = 1;
    }
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        metadata: {
          ...
          userId: session.user.email,
          oldFilePath: oldFilePath // Track old file for debugging
        },
    ...
    return NextResponse.json({
      ...
      // 🔧 FIX: Return additional info for debugging
      debug: {
