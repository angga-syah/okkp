...
const setupRealtimeSubscription = useCallback(() => {
  ...
  try {
    // 🔧 FIX: More conservative subscription setup
    ...
    const channel = supabaseClient
      ...
      .subscribe((status, err) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          if (reconnectAttempts.current < 2) { // Reduced from 3
            const delay = 5000; // Fixed 5 second delay
            reconnectAttempts.current++;
            
            console.log(`🔄 Will retry subscription in ${delay}ms (attempt ${reconnectAttempts.current}/2)`);
