// src/app/api/orders/optimized/route.ts - FIXED VERSION FOR EXPORT
...
// 🔧 FIXED: Enhanced numeric validation with export support
const validateNumericInput = (
  input: string | null, 
  min: number, 
  max: number, 
  isExportRequest?: boolean // Make parameter optional with explicit undefined type
): number => {
  ...
  // 🔧 FIX: Special handling for export requests
  if (isExportRequest === true) {
    const exportMaxLimit = 50000; // Maximum records for export
    if (num > exportMaxLimit) {
      console.log(`📊 Export limit capped: ${num} → ${exportMaxLimit}`);
      return exportMaxLimit;
    }
    console.log(`📊 Export request detected: limit=${num}`);
    return num;
  }
  
  // 🔧 FIX: For regular requests, cap at max instead of returning min
  if (num > max) {
    console.log(`📊 Regular limit capped: ${num} → ${max}`);
    return max;
  }
...
export async function GET(request: NextRequest): Promise<NextResponse<OrdersResponse | { error: string; message: string; code: string; timestamp: string }>> {
  ...
  try {
    ...
    // 🔧 FIX: Detect export requests based on high limit values with proper boolean conversion
    const isExportRequest: boolean = Boolean(rawLimit && parseInt(rawLimit, 10) > 1000);
    
    console.log(`📊 Request type: ${isExportRequest ? 'EXPORT' : 'REGULAR'} (limit: ${rawLimit})`);

    // 🔧 FIXED: Enhanced numeric validation with export support
    const page = validateNumericInput(rawPage, 1, 10000);
    const limit = validateNumericInput(rawLimit, 1, 1000, isExportRequest); // 🔧 FIX: Support export limits

    console.log(`📊 Processed params: page=${page}, limit=${limit}, isExport=${isExportRequest}`);
    ...
    // Sort direction validation with explicit boolean conversion
    const sortDirection: 'asc' | 'desc' = (rawSortDirection?.toLowerCase() === 'asc') ? 'asc' : 'desc';
    ...
    // Enhanced day/month/year validation with explicit undefined handling
    ...
    if (year && !month && !day) {
      ...
      const monthStart = `${currentYearForFilter}-${monthStr}-01T00:00:00.000Z`;
      
      // Calculate last day of month
      const lastDay = new Date(currentYearForFilter, month, 0).getDate();
      const monthEnd = `${currentYearForFilter}-${monthStr}-${lastDay.toString().padStart(2, '0')}T23:59:59.999Z`;
      
      supabaseQuery = supabaseQuery.gte('created_at', monthStart).lte('created_at', monthEnd);
      console.log('🔍 Applied month-only filter (current year):', `${currentYearForFilter}-${monthStr}`);
      ...
      const dayStr = day.toString().padStart(2, '0');
      const monthStr = month.toString().padStart(2, '0');
      const dayStart = `${currentYearForFilter}-${monthStr}-${dayStr}T00:00:00.000Z`;
      const dayEnd = `${currentYearForFilter}-${monthStr}-${dayStr}T23:59:59.999Z`;
      supabaseQuery = supabaseQuery.gte('created_at', dayStart).lte('created_at', dayEnd);
      console.log('🔍 Applied day-month filter (current year):', `${currentYearForFilter}-${monthStr}-${dayStr}`);
    ...
    if (sortField === 'created_at') {
      supabaseQuery = supabaseQuery.order('created_at', { ascending: isAscending });
    } else if (sortField === 'status') {
      // Use composite index for status + created_at
      supabaseQuery = supabaseQuery.order('status', { ascending: isAscending })
                                   .order('created_at', { ascending: false });
    } else if (sortField === 'service_name') {
      // Use composite index for service + created_at
      supabaseQuery = supabaseQuery.order('service_name', { ascending: isAscending })
                                   .order('created_at', { ascending: false });
    } else {
      // For other fields, add created_at as secondary sort
      supabaseQuery = supabaseQuery.order(sortField, { ascending: isAscending })
                                   .order('created_at', { ascending: false });
    }

    // 🔧 FIX: Apply pagination with proper range calculation
    console.log(`📊 Applying pagination: offset=${offset}, limit=${limit}, range=[${offset}, ${offset + limit - 1}]`);
    ...
    try {
      ...
      console.log(`📊 Query executed: ${queryTime}ms, returned ${orders?.length || 0} records, total count: ${count || 0}`);
