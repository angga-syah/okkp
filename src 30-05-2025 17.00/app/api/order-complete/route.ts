  ...
  try {
    ...
    try {
      ...
      const invoiceData = {
        ...
        successRedirectUrl: process.env.NODE_ENV === 'development' 
          ? `${process.env.NEXT_PUBLIC_BASE_URL}/payment-success?demo=true&invoice_id=demo-${Date.now()}&amount=${validatedAmount}`
          : `${process.env.NEXT_PUBLIC_BASE_URL}/payment-success?invoice_id={invoice_id}&external_id={external_id}&payment_method={payment_method}&paid_amount={paid_amount}`,
        failureRedirectUrl: `${process.env.NEXT_PUBLIC_BASE_URL}/payment-failed?external_id={external_id}`
      };
